...
class ToolWalletTest(ElixirTestFramework):
    ...
    def elixir_wallet_process(self, *args):
        binary = self.config["environment"]["BUILDDIR"] + '/src/elixir-wallet' + self.config["environment"]["EXEEXT"]
    ...
    def assert_raises_tool_error(self, error, *args):
        p = self.elixir_wallet_process(*args)
    ...
    def do_tool_createfromdump(self, wallet_name, dumpfile, file_format=None):
        ...
        self.assert_tool_output("The dumpfile may contain private keys. To ensure the safety of your Elixir, do not share the dumpfile.\n", '-wallet={}'.format(wallet_name), '-dumpfile={}'.format(rt_dumppath), 'dump')
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `elixir-wallet help` raises an error. Use `elixir-wallet -help`.
        ...
        self.assert_raises_tool_error('No method provided. Run `elixir-wallet -help` for valid methods.')
        self.assert_raises_tool_error('Wallet name must be provided when creating a new wallet.', 'create')
        locked_dir = os.path.join(self.options.tmpdir, "node0", "regtest", "wallets")
        error = 'Error initializing wallet database environment "{}"!'.format(locked_dir)
        if self.options.descriptors:
            error = "SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another elixird?"
    ...
    def test_salvage(self):
        # TODO: Check salvage actually salvages and doesn't break things. https://github.com/elixir/elixir/issues/7463
    ...
    def test_dump_createfromdump(self):
        ...
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of elixir-wallet only supports version 1 dumpfiles. Got dumpfile with version 0', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        ...
        self.write_dump(dump_data, bad_ver_wallet_dump)
        self.assert_raises_tool_error('Error: Dumpfile version is not supported. This version of elixir-wallet only supports version 1 dumpfiles. Got dumpfile with version 2', '-wallet=badload', '-dumpfile={}'.format(bad_ver_wallet_dump), 'createfromdump')
        assert not os.path.isdir(os.path.join(self.nodes[0].datadir, "regtest/wallets", "badload"))
