...
const char * const BITCOIN_CONF_FILENAME = "elixir.conf";
 ...
 * https://github.com/elixir/elixir/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //elixir-tx using stdin
        std::string val;
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Elixir
    // macOS: ~/Library/Application Support/Elixir
    // Unix-like: ~/.elixir
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Elixir";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Elixir";
#else
    // Unix-like
    return pathRet / ".elixir";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Elixir Core copyright is not removed by accident
    if (copyright_devs.find("Elixir Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Elixir Core developers";
    }
