dnl Copyright (c) 2013-2016 The Elixir Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

dnl Helper for cases where a qt dependency is not met.
dnl Output: If qt version is auto, set elixir_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$elixir_qt_want_version" = xauto && test "x$elixir_qt_force" != xyes; then
    if test "x$elixir_enable_qt" != xno; then
      AC_MSG_WARN([$1; elixir-qt frontend will not be built])
    fi
    elixir_enable_qt=no
    elixir_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "x$elixir_enable_qt" != xno && test "x$elixir_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build elixir-qt GUI (default=auto)])],
    [
     elixir_qt_want_version=$withval
     if test "x$elixir_qt_want_version" = xyes; then
       elixir_qt_force=yes
       elixir_qt_want_version=auto
     fi
    ],
    [elixir_qt_want_version=auto])
  ...
  AS_IF([test "x$with_gui" = xqt5_debug],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); elixir_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl Outputs: elixir_enable_qt, elixir_enable_qt_dbus, elixir_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "x$elixir_cv_static_qt" = xyes; then
  ...
  BITCOIN_QT_CHECK([
    elixir_enable_qt=yes
    elixir_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      elixir_enable_qt_test=no
    fi
    elixir_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      elixir_enable_qt_dbus=yes
    fi
  ...
  if test x$elixir_enable_qt = xyes; then
    AC_MSG_RESULT([$elixir_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$elixir_enable_qt])
  fi
...
dnl Output: elixir_cv_static_qt=yes|no
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, elixir_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [elixir_cv_static_qt=yes],
      [elixir_cv_static_qt=no])
    ])
])
