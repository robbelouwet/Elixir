# Fuzzing Elixir Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Elixir Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/elixir/elixir
$ cd elixir/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/elixir/elixir/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/elixir/elixir/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/elixir/elixir/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/elixir/elixir/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`elixir-core/qa-assets`](https://github.com/elixir-core/qa-assets) repo.

To fuzz `process_message` using the [`elixir-core/qa-assets`](https://github.com/elixir-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/elixir-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`elixir-core/qa-assets`](https://github.com/elixir-core/qa-assets) repo.

Every single pull request submitted against the Elixir Core repo is automatically tested against all inputs in the [`elixir-core/qa-assets`](https://github.com/elixir-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Elixir Core more robust.
...
with certain assembly code from Elixir Core's code. See [developer notes on sanitizers](https://github.com/elixir/elixir/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing Elixir Core using afl++

## Quickstart guide

To quickly get started fuzzing Elixir Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Elixir Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Elixir Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Elixir Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Elixir
Core without having to write any custom fuzzing harness. The `elixird` server
...
```sh
$ mkdir elixir-honggfuzz-p2p/
$ cd elixir-honggfuzz-p2p/
$ git clone https://github.com/elixir/elixir
$ cd elixir/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/elixird.cpp b/src/elixird.cpp
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/elixird -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
...
Elixir Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/elixir-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=elixir-core).
...
because Elixir is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_elixir-core/latest).
