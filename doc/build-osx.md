...
This guide describes how to build elixird, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Elixir Core from source.
...
### 4. Clone Elixir repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Elixir Core repository to a directory.
...
It is not necessary to build wallet functionality to run `elixird` or  `elixir-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/elixir/elixir/blob/master/doc/descriptors.md), `sqlite` is required.
...
Elixir Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/elixir/elixir/issues/7714).
...
You can deploy a `.dmg` containing the Elixir Core application using `make deploy`.
...
## Building Elixir Core

### 1. Configuration

There are many ways to configure Elixir Core, here are a few common examples:
...
Run the following in your terminal to compile Elixir Core:
...
Elixir Core should now be available at `./src/elixird`.
If you compiled support for the GUI, it should be available at `./src/qt/elixir-qt`.

The first time you run `elixird` or `elixir-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Elixir/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Elixir"

touch "/Users/${USER}/Library/Application Support/Elixir/elixir.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Elixir/elixir.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Elixir/debug.log
```

## Other commands:

```shell
./src/elixird -daemon      # Starts the elixir daemon.
./src/elixir-cli --help    # Outputs a list of command-line options.
./src/elixir-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/elixir-qt -server # Starts the elixir-qt server mode, allows elixir-cli control
```
