...
The Elixir Core daemon can be configured to act as a trusted "border
router", implementing the elixir wire protocol and relay, making
...
The ZeroMQ feature in Elixir Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building elixird:
...
For instance:
    ...
    $ elixird -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/elixird.tx.raw \
...
These options can also be provided in elixir.conf.
...
From the perspective of elixird, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into elixird directly. Furthermore, no information is
...
using. Elixird appends an up-counting sequence number to each
