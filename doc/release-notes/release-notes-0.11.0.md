Elixir Core version 0.11.0 is now available from:

  <https://elixir.org/bin/elixir-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer ELXR/kB fee to be
...
For example, add the following to `elixir.conf`:
...
related to the blockchain in the elixir system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Elixir Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Elixir Core will 
...
For an example script see [elixir-submittx](https://github.com/laanwj/elixir-submittx).
...
- #5334 `cf87536` libelixirconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new elixir-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only ElixirAmountField
- #5753 `0f44672` Add elixir logo to about screen
...
- #5809 `46bfbe7` Add elixir-cli man page
...
- #5849 `41259ca` contrib/init/elixird.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Elixir-Qt.app to Elixir-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for elixir-tx)
