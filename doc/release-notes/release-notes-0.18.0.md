Elixir Core version 0.18.0 is now available from:

  <https://elixircore.org/bin/elixir-core-0.18.0/>
...
`/Applications/Elixir-Qt` (on Mac) or `elixird`/`elixir-qt` (on
Linux).
...
Elixir Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Elixir Core on unsupported systems.

Elixir Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Elixir Core does not yet change appearance when
...
`contrib/init/elixird.service`, it has been changed to use
`/var/lib/elixird` as the data directory instead of
`~elixir/.elixir`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/elixird` will
exist has enough space (check using `df -h /var/lib/elixird`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/elixir/elixir/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://elixircore.org/en/contact/) the Elixir Core
...
The systemd init file (`contrib/init/elixird.service`) has been changed
to use `/var/lib/elixird` as the data directory instead of
`~elixir/.elixir`. This change makes Elixir Core more consistent with
...
`datadir` option in `/etc/elixir/elixir.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/elixir/elixir.conf`.


Documentation
-------------

- A new short [document](https://github.com/elixir/elixir/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/elixir/elixir/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/elixir/elixir/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/elixir/elixir/blob/master/doc/elixir-conf.md)
  about the `elixir.conf` file describes how to use it to configure
  Elixir Core.

- A new document introduces Elixir Core's BIP174 [Partially-Signed
  Elixir Transactions
  (PSBT)](https://github.com/elixir/elixir/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/elixir/elixir/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Elixir-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Elixir Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/elixir/elixir/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/elixir/elixir/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Elixir Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Elixir Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/elixir/elixir/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/elixir/elixir/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart elixird with the `-deprecatedrpc=generate`
...
- A new `elixir-wallet` tool is now distributed alongside Elixir
...
This section describes planned changes to Elixir Core that may affect
other Elixir software and services.

- Since version 0.16.0, Elixir Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Elixir Core 0.20 (expected about
  a year after 0.18), Elixir Core will default to native segwit
  ...
  to bech32 addresses, and if the Elixir Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Elixir Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Elixir Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Elixir Core
...
- A sub-project of Elixir Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Elixir Core.  See their [project
  page](https://github.com/elixir-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Elixir Core's own implementation, although entropy
  gathered by Elixir Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Elixir Core a
...
- On macOS, Elixir Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `elixir_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libelixir_server.a` (jnewbery)
...
- #13926 [Tools] elixir-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like elixird (MarcoFalke)
...
- #14521 qt, docs: Fix `elixir-qt -version` output formatting (hebasto)
...
- #11625 Add ElixirApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libelixir_server` (laanwj)
...
- #15154 Configure: elixir-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/elixir-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Elixir Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libelixir_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in elixir-util-test.py (isghe)
...
- #12775 Integration of property based testing into Elixir Core (Christewart)
...
- #12246 Bugfix: Only run elixir-tx tests when elixir-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_elixir` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_elixir` (MarcoFalke)
...
- #14186 elixir-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_elixir.conf` message on startup if conf file exists (leishman)
...
- #12255 Update elixir.service to conform to init.md (dongcarl)
...
- #14497 Add doc/elixir-conf.md (hebasto)
...
- #15354 Add missing `elixir-wallet` tool manpages (MarcoFalke)
...
- #15765 correct elixirconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support elixir folder on OSX (JimmyMow)
