...
Elixir Core version 0.20.0 is now available from:

  <https://elixircore.org/bin/elixir-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Elixir Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Elixir Core version 0.18.0.
...
- Testing or debugging of implementations of the Elixir P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Elixir Core. Elixir Core logs debug messages
...
- OpenSSL is no longer used by Elixir Core.  (#17265)

- BIP70 support has been fully removed from Elixir Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/elixir/elixir/issues/16599),
  [PR #16702](https://github.com/elixir/elixir/pull/16702), and the
  `elixird help` for more information.  This option is experimental and
...
- All custom settings configured when Elixir Core starts are now
...
- The "Start Elixir Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://elixircore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Elixir Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Elixir Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/elixir-core/HWI) for
  signing.  Future versions of Elixir Core should support a GUI option
...
- Elixir Core's automatically-generated source code documentation is
  now available at https://doxygen.elixircore.org.  (#17596)
...
- The `elixir-cli` utility used with the `-getinfo` parameter now
...
- The `elixir-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `elixirstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_elixir-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_elixir` binary (hebasto)
...
- #18702 Fix ASLR for elixir-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate elixir schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest elixir.conf (sdaftuar)
...
- #18022 Fix appveyor `test_elixir` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (elixir-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone elixir-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Elixir Core files (hebasto)
...
- #17119 Fix broken elixir-cli examples (andrewtoth)
...
- #17503 Remove bitness from elixir-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.elixircore.org (laanwj)
...
- #17648 Rename wallet-tool references to elixir-wallet (hel-o)
...
- #18341 Replace remaining literal ELXR with `CURRENCY_UNIT` (domob1812)
...
- #16115 On elixird startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from elixir-cli -getinfo (malevolent)
